...
os.chdir(r"C:\Users\abdle\OneDrive\Desktop\ThesisModel") 
...
rib_1_4_mp = (width/2.0, height/2.0, 0.0) 
rib_2_3_5_6_mp = (lenght/2.0, height/2.0, 0.0)
...
lenght_p = dist_2 * 2.0 * n_z #mm
plate_t = 4.0 #mm 

plate_md = (width_p/2.0, lenght_p/2.0, 0.0)
...
mdb.models['Model-1'].parts['Part-1'].SectionAssignment(offset=0.0, 
    ...
    faces=mdb.models['Model-1'].parts['Part-1'].faces.findAt(((width/2.0, 
    height/2.0, 0.0), (0.0, 0.0, 1.0)), )), sectionName='Section-1', 
    thicknessAssignment=FROM_SECTION)
    
mdb.models['Model-1'].parts['Part-2'].SectionAssignment(offset=0.0, 
    offsetField='', offsetType=MIDDLE_SURFACE, region=Region(
    faces=mdb.models['Model-1'].parts['Part-2'].faces.findAt(((lenght/2.0, 
    height/2.0, 0.0), (0.0, 0.0, 1.0)), )), sectionName='Section-1', 
    thicknessAssignment=FROM_SECTION)
...
mdb.models['Model-1'].parts['Part-3'].SectionAssignment(offset=0.0, 
    offsetField='', offsetType=MIDDLE_SURFACE, region=Region(
    faces=mdb.models['Model-1'].parts['Part-3'].faces.findAt(((width_p/2.0, 
    lenght_p/2.0, 0.0), (0.0, 0.0, 1.0)), )), sectionName='Section-1', 
    thicknessAssignment=FROM_SECTION)
...
mdb.models['Model-1'].rootAssembly.rotate(angle=-theta, axisDirection=(0.0, 
    250.0, 0.0), axisPoint=(-dist_3, 0.0, dist_2), instanceList=('Part-2-2', 
    ))
...
mdb.models['Model-1'].rootAssembly.rotate(angle=-theta, axisDirection=(0.0, 
    250.0, 0.0), axisPoint=(width, 0.0, 0.0), instanceList=('Part-2-3', ))
...
mdb.models['Model-1'].rootAssembly.translate(instanceList=('Part-2-4', ), 
    vector=(dist_3 - 10.0, 0.0, dist_2))
    
mdb.models['Model-1'].rootAssembly.rotate(angle=theta, axisDirection=(0.0, 
    250.0, 0.0), axisPoint=(width + dist_3 , 0.0, dist_2), instanceList=('Part-2-4', 
    ))
...
mdb.models['Model-1'].DisplacementBC(amplitude=UNSET, createStepName='Step-1', 
    ...
    yMin=height/2.0, zMin=-1.0, xMax=width_p+1.0, yMax=height+1.0, zMax=lenght_p+1.0)), u1=UNSET, u2=-height*0.1, u3=
...
mdb.models['Model-1'].DisplacementBC(amplitude=UNSET, createStepName='Step-1', 
    ...
    yMin=-1.0, zMin=-1.0, xMax=width_p+1.0, yMax=height/2.0, zMax=lenght_p+1.0)), 
...
mdb.models['Model-1'].rootAssembly.seedPartInstance(deviationFactor=0.1, 
    ...
    mdb.models['Model-1'].rootAssembly.instances['Part-4-1'], ), size= 30.0)
...
mdb.models['Model-1'].rootAssembly.Set(name='Set-1', nodes=
    mdb.models['Model-1'].rootAssembly.instances['Part-4-1'].nodes.getByBoundingBox(xMin=-1.0, yMin=-1.0, zMin=-1.0, xMax=width_p+1.0, yMax=1.0, zMax=lenght_p+1.0))
...
opFile = BaseDir+"/"+str(Foldername)+'/'+'DatainExcel.csv'  
...
for i in range(NrOfSteps):
    ...
    if i!=0 and displacements[i-1]!= displacements[i]:
        ...
        Stiffness.append(Current_Stiffness)
    opFileU.write("%10f,%10f,%10f\n" % (displacement, force, Current_Stiffness))
opFileU.close()     
